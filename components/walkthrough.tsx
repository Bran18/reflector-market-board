...
export function Walkthrough({ contractId }: { contractId?: string }) {
  return (
    <div className="min-w-0 space-y-10 text-sm leading-6 text-zinc-600 dark:text-zinc-400">
      <section
      ...
      >
        ...
        <p>
          ...
          <code>REFLECTOR_PULSE_CONTRACT_ID</code> in your server environment.
          The example environment selects Stellar Pubnet Pulse; explore{" "}
        ...
        >
          ...
          <pre className="mt-4 overflow-x-auto text-xs leading-6" tabIndex={0}>
            <code>{clientExample}</code>
          </pre>
          <p className="mt-3">
            <code>requiredEnv()</code> is the local helper that validates
            required settings. <code>getPulseClient()</code> creates this client
            once and reuses it.
          </p>
        </details>
        <pre
        ...
        <p>
          ...
          shared metadata. <code>findOracleAsset()</code> matches a configured
          symbol to an actual listed asset. If <code>lastPrice()</code> returns
          a record, <code>getMarketAsset()</code> formats it, and the async{" "}
          <code>MarketCardSlot</code> Server Component passes it to{" "}
          <code>MarketCard</code>. Missing feeds and empty records get explicit
          fallback cards.
        </p>
        <p>
          Open any card’s <strong>Oracle details</strong> for its raw integer,
          decimals, exact conversion, UTC timestamp, and full contract ID. Feed
          resolution is the oracle’s interval, not the age of an individual
          price. Ages are calculated when the server renders; reload to fetch
          again.
        </p>
      </section>
      ...
      <section aria-labelledby="no-wallet" className="space-y-3">
        ...
        <p>
          These public Pulse reads simulate contract calls through Soroban RPC.
          They do not sign or submit transactions. The SDK needs a public
          G-address to build the simulation envelope, but no secret key,
          Freighter connection, or signing callback is needed. Next.js performs
          the reads on the server and renders the results for your browser.
        </p>
      </section>
      ...
      >
        ...
        <ol className="list-decimal space-y-2 pl-5">
          <li>
            Inspect <code>await getPulseClient().assets()</code> on the server,
            or use the{" "}
            <a href={LINKS.pulse} className="underline underline-offset-4">
              Pulse asset list
            </a>
          ...
          <li>
            ...
            matching entry to <code>FEATURED_ASSET_REGISTRY</code> in the same
            file. For a <code>Stellar</code> asset, put its exact C-address in{" "}
            <code>stellarContracts</code>; for an <code>Other</code> asset, put
            its exact ticker in <code>otherTickers</code>. Use an empty array
            for the unused kind. A ticker alone will not match a Stellar
            contract address.
          </li>
          <li>
            Follow the{" "}
          ...
          <li>
            Confirm the new card returns a price and inspect its raw value. If
            the feed is unavailable, check the identifier against{" "}
            <code>assets()</code> on the same contract; an empty{" "}
            <code>lastPrice()</code> means no current record.
          </li>
        </ol>
        <p>
          The grid uses the same client methods for every configured asset. No
          new data source or UI logic is needed.
        </p>
      </section>
    </div>
  );
